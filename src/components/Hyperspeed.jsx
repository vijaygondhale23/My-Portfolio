...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    const roadMarkings_fragment = `
      ...
      float brokenLines = step(1.0 - brokenLineWidth, fract(uv.x * 2.0)) * step(1. - uBrokenLinesLengthPercentage, fract(uv.y * 10.0));
    ...
    class App {
      ...
      tick() {
        ...
        if (this.isPaused) return; // Skip updating and rendering if paused
    ...
    (function () {
      ...
      // Add Intersection Observer to pause rendering when off-screen
      observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (myApp) {
            myApp.isPaused = !entry.isIntersecting;
          }
        });
      }, { rootMargin: '100px' });
